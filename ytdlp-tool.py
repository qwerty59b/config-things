### Dependencias ###
# sudo pacman -S python-pip python-requests tk ffmpeg  #Arch
# sudo apt install python3-tk python3-pip python3-requests ffmpeg # Debian/Ubuntu
# sudo dnf install python3-tkinter python3-pip python3-requests ffmpeg  # Fedora
...
class YTDownloaderApp:
    ...
    def check_ffmpeg_installed(self):
        ...
        try:
            # Primero intenta usar la ruta configurada
            ffmpeg_path = self.ffmpeg_path.get()
            if ffmpeg_path and os.path.exists(ffmpeg_path):
                return True
                
            # Luego verifica en el PATH del sistema
            if sys.platform == "win32":
    ...
    def load_config(self):
        ...
        if sistema == "Windows":
            ...
            default_ffmpeg = os.path.join(os.getcwd(), "ffmpeg.exe")
        else:
        ...
        try:
            if CONFIG_PATH.exists():
                with open(CONFIG_PATH, "r") as f:
                    ...
                    self.ffmpeg_path.set(config.get("ffmpeg_path", default_ffmpeg))
    ...
    def create_widgets(self):
        ...
        # Path de FFmpeg
        ttk.Label(config_frame, text="Ruta de FFmpeg:").grid(row=1, column=0, sticky="w", padx=5, pady=2)
        self.ffmpeg_entry = ttk.Entry(config_frame, textvariable=self.ffmpeg_path, width=50)
        self.ffmpeg_entry.grid(row=1, column=1, padx=5, pady=2)
        ttk.Button(config_frame, text="Examinar", command=self.browse_ffmpeg).grid(row=1, column=2, padx=5, pady=2)
        
        # Botón para instalar/descargar FFmpeg
        ffmpeg_btn_text = "Descargar FFmpeg" if sys.platform == "win32" else "Instalar FFmpeg"
        self.ffmpeg_btn = ttk.Button(
            config_frame, 
            text=ffmpeg_btn_text, 
            command=self.show_install_ffmpeg
        )
        self.ffmpeg_btn.grid(row=1, column=3, padx=5, pady=2)
        
        # Indicador de estado FFmpeg
        self.ffmpeg_status = ttk.Label(config_frame, text="FFmpeg: " + ("✓ Instalado" if self.ffmpeg_installed else "✗ No encontrado"))
        self.ffmpeg_status.grid(row=1, column=4, columnspan=2, padx=5, pady=2)
        
        # Carpeta destino
        ttk.Label(config_frame, text="Carpeta Destino:").grid(row=2, column=0, sticky="w", padx=5, pady=2)
        ttk.Entry(config_frame, textvariable=self.output_folder, width=50).grid(row=2, column=1, padx=5, pady=2)
        ttk.Button(config_frame, text="Examinar", command=self.browse_output).grid(row=2, column=2, padx=5, pady=2)
        
        # Descargas simultáneas
        ttk.Label(config_frame, text="Descargas Simultáneas:").grid(row=3, column=0, sticky="w", padx=5, pady=2)
        ttk.Spinbox(config_frame, from_=1, to=10, textvariable=self.max_simultaneous, width=5).grid(row=3, column=1, sticky="w", padx=5, pady=2)
        
        # Auto eliminar al completar
        ttk.Checkbutton(config_frame, text="Eliminar al completar", variable=self.auto_remove).grid(row=3, column=2, padx=10, pady=2)
        
        # Reintentos
        ttk.Label(config_frame, text="Reintentos:").grid(row=3, column=3, sticky="e", padx=(10,5), pady=2)
        ttk.Spinbox(config_frame, from_=0, to=20, textvariable=self.retry_attempts, width=5).grid(row=3, column=4, sticky="w", padx=5, pady=2)
        
        # Fragmentos concurrentes
        ttk.Label(config_frame, text="Fragmentos Concurrentes:").grid(row=3, column=5, sticky="e", padx=(10,5), pady=2)
        ttk.Spinbox(
            config_frame, 
            from_=1, 
            to=16, 
            ...
            width=5
        ).grid(row=3, column=6, sticky="w", padx=5, pady=2)
    ...
    def show_usage_guide(self):
        ...
        content = [
            ...
            ("   - Después de descargarlo, usa 'Examinar' para seleccionar su ubicación.",
             11, False),
            ("2. Instalar FFmpeg:", 12, True),
            ("   - Para ciertas operaciones (como extraer audio) necesitarás FFmpeg.",
             11, False),
            ("   - En Windows: usa 'Descargar FFmpeg' para obtener el ejecutable.",
             11, False),
            ("   - En Linux: usa 'Instalar FFmpeg' para ver instrucciones de instalación.",
             11, False),
            ("3. Configurar carpeta destino:", 12, True),
    ...
    def show_install_ffmpeg(self):
        ...
        if sistema == "Windows":
            ...
            content = [
                ("Instalación de FFmpeg en Windows", 14, True),
                ("1. Descargar:", 12, True),
                ("   - Visita el sitio oficial:", 11, False),
                ("     https://www.gyan.dev/ffmpeg/builds/", 11, False),
                ("   - Descarga la última versión 'full'", 11, False),
                ("2. Extraer:", 12, True),
                ("   - Extrae el archivo .7z descargado", 11, False),
                ("3. Configurar:", 12, True),
                ("   - Busca el archivo ffmpeg.exe dentro de la carpeta bin", 11, False),
                ("   - Usa el botón 'Examinar' para seleccionarlo", 11, False),
            ]
        else:
            content = [
                ("Instalación de FFmpeg en Linux", 14, True),
                ("Instala FFmpeg usando tu gestor de paquetes:", 12, True),
                ("- Debian/Ubuntu:", 11, True),
                ("   sudo apt update && sudo apt install ffmpeg", 11, False),
                ("- Arch Linux/Manjaro:", 11, True),
                ("   sudo pacman -S ffmpeg", 11, False),
                ("- Fedora:", 11, True),
                ("   sudo dnf install ffmpeg", 11, False),
                ("Después de instalar, haz clic en 'Verificar FFmpeg'", 12, True),
            ]
        ...
        if sistema == "Windows":
            ...
            ttk.Button(btn_frame, text="Abrir sitio de descarga", command=open_ffmpeg_site).pack(side="left", padx=5)
        
        def verify_ffmpeg():
            self.ffmpeg_installed = self.check_ffmpeg_installed()
            self.ffmpeg_status.config(text="FFmpeg: " + ("✓ Instalado" if self.ffmpeg_installed else "✗ No encontrado"))
            if self.ffmpeg_installed:
                messagebox.showinfo("Verificación", "FFmpeg encontrado correctamente")
            else:
                messagebox.showerror("Error", "FFmpeg no encontrado. Por favor instálalo correctamente.")
        
        ttk.Button(btn_frame, text="Verificar FFmpeg", command=verify_ffmpeg).pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Cerrar", command=dialog.destroy).pack(side="left", padx=5)
    ...
    def browse_ffmpeg(self):
        ...
        filepath = filedialog.askopenfilename(
            title="Seleccionar FFmpeg",
            filetypes=filetypes
        )
        if filepath:
            self.ffmpeg_path.set(filepath)
            # Verificar si el nuevo path es válido
            self.ffmpeg_installed = self.check_ffmpeg_installed()
            self.ffmpeg_status.config(text="FFmpeg: " + ("✓ Instalado" if self.ffmpeg_installed else "✗ No encontrado"))
    ...
    def add_to_queue(self):
        ...
        # Verificar si es descarga de audio y si FFmpeg está disponible
        if "audio" in resolution.lower() and not self.ffmpeg_installed:
            if not messagebox.askyesno(
                "FFmpeg requerido", 
                "Para descargar solo audio se necesita FFmpeg. ¿Desea continuar igualmente?\n"
                "La descarga podría fallar si no está instalado."
            ):
                return
    ...
    def start_downloads(self):
        ...
        # Verificar FFmpeg para descargas de audio
        audio_downloads = False
        for item in self.dl_tree.get_children():
            values = self.dl_tree.item(item, "values")
            if values and len(values) >= 3 and "audio" in values[2].lower():
                audio_downloads = True
                break
                
        if audio_downloads and not self.ffmpeg_installed:
            if not messagebox.askyesno(
                "FFmpeg requerido", 
                "Algunas descargas requieren FFmpeg. ¿Desea continuar igualmente?\n"
                "Las descargas de audio podrían fallar si no está instalado."
            ):
                return
    ...
    def start_single_download(self, item):
        ...
        # Añadir FFmpeg si está configurado
        ffmpeg_path = self.ffmpeg_path.get()
        if ffmpeg_path and os.path.exists(ffmpeg_path):
