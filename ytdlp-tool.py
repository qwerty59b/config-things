...
class YTDownloaderApp:
    def __init__(self, root):
        ...
        self.resolution = tk.StringVar(value="best")  # Nueva variable para resolución
        ...
        try:
            if CONFIG_PATH.exists():
                with open(CONFIG_PATH, "r") as f:
                    ...
                    self.resolution.set(config.get("resolution", "best"))  # Cargar resolución
            else:
                self.ytdlp_path.set(default_ytdlp)
                self.output_folder.set(str(default_output))
                self.resolution.set("best")  # Valor por defecto
    ...
    def save_config(self):
        config = {
            ...
            "resolution": self.resolution.get()  # Guardar resolución
        }
    ...
    def create_widgets(self):
        ...
        # Nueva opción: Resolución
        ttk.Label(config_frame, text="Resolución:").grid(row=2, column=3, sticky="e", padx=(10,5), pady=2)
        resolution_combo = ttk.Combobox(config_frame, textvariable=self.resolution, width=12)
        resolution_combo.grid(row=2, column=4, sticky="w", padx=5, pady=2)
    ...
    def run_download(self, cmd, item):
        try:
            sistema = platform.system()
            if sistema != "Windows":
            ...
            self.complete_download(item, process.returncode)
