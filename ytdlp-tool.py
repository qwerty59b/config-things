...
class YTDownloaderApp:
    ...
    def show_usage_guide(self):
        ...
        content = [
            ...
            ("   - Después de descargarlo, usa 'Examinar' para seleccionar su ubicación.",
             11, False),
            ("   - En Linux: Debes hacer el archivo ejecutable con:", 11, False),
            ("     chmod +x /ruta/a/yt-dlp", 11, True),
            ("", 1, False),
    ...
    def check_ytdlp_update(self):
        ...
        try:
            self.root.after(0, lambda: self.status_var.set("Verificando actualizaciones..."))
            ytdlp_path = self.ytdlp_path.get()
            if not os.path.exists(ytdlp_path):
                self.root.after(0, lambda: self.status_var.set("yt-dlp no encontrado"))
                return
            ...
            try:
                ...
                self.root.after(0, lambda: self.status_var.set("Error al verificar actualización"))
            # Comparar versiones
            if local_version != latest_version:
                self.new_version_available = True
                self.root.after(0, self.update_update_button_style)
                self.root.after(0, lambda: messagebox.showinfo("Actualización disponible", 
    ...
    def run_update_command(self, cmd):
        ...
        try:
            self.root.after(0, lambda: self.status_var.set("Actualizando yt-dlp..."))
            # Preparar el proceso
            sistema = platform.system()
            if sistema != "Windows":
            ...
            while True:
                ...
                if line:
                    ...
                    self.root.after(0, lambda l=line: self.status_var.set(l.strip()))
            # Mostrar resultado
            returncode = process.returncode
            output = "\n".join(output_lines)
            if returncode == 0:
                self.root.after(0, lambda: messagebox.showinfo("Actualización completada", 
                                   f"yt-dlp se ha actualizado correctamente:\n{output}"))
                self.new_version_available = False
                self.root.after(0, self.update_update_button_style)
            else:
                self.root.after(0, lambda: messagebox.showerror("Error en actualización", 
                                    f"Error al actualizar yt-dlp (código {returncode}):\n{output}"))
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Error", f"Excepción al actualizar yt-dlp:\n{str(e)}"))
        finally:
            self.root.after(0, lambda: self.status_var.set("Listo"))
        ...
        while attempts <= max_retries and not success:
            ...
            try:
                ...
                if attempts <= max_retries:
                    ...
                    self.root.after(0, lambda: self.dl_tree.item(item, values=(
    ...
    def complete_download(self, item, returncode):
        ...
        if returncode == 0:
            status = "Completado"
            self.root.after(0, lambda: self.status_var.set(f"Descarga completada: {custom_name or url}"))
            # Eliminar automáticamente si está habilitado
            if self.auto_remove.get():
                self.root.after(0, lambda: self.dl_tree.delete(item))
            else:
                self.root.after(0, lambda: self.dl_tree.item(item, values=(url, custom_name, resolution, status)))
        else:
            status = "Fallido"
            self.root.after(0, lambda: self.dl_tree.item(item, values=(url, custom_name, resolution, status)))
        # Eliminar de activos
